...
**** blanket
The tenet 'tested' doesn't look at the function call. Only the name of the testing function and compares it to the name of the function in the other file, which is definitely not the same as blanket.
It can, however, find if a method with the _name_ 'TestMethod' was called (with an arbitrarily deep recusive nesting) in the '_test.go' file, where seeing if a test method for 'Method' exists.
