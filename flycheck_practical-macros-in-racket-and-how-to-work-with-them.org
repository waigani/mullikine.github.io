...
*** ~with-syntax~ -- similar-to but should be used alongside ~syntax-case~
...
The *result* of the ~with-syntax~ form is the result of the _last body_, which is in tail position with respect to the ~with-syntax~ form.
