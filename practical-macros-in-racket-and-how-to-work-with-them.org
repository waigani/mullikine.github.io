...
*** ~with-syntax~ very similar to /(but should be used alongside)/ ~syntax-case~
...
This is awkwardly under the ~syntax-case~ section. ~with-syntax~ is the only thing here from the ~syntax-case~ family.
