...
** Example A: (The most concise way). Uses ~define-syntax-parser~
Most of the time we want to use ~define-syntax-parser~ because it saves us the effort of typing ~syntax-parse~ and ~stx~.
...
** Perfectly fine example B: (The most expanded way)
#+BEGIN_SRC racket
...
** Perfectly fine example C: (shorthand, expanded)
